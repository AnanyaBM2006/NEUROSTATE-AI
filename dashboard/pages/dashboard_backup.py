...
def estimate_age_simulation(
    attention,
    alpha,
    beta
):
    """
    TEMPORARY STEP-A SIMULATION.

    In STEP B this function will be replaced by
    the actual trained age-estimation model.
    """
...
def show_dashboard():
    ...
    with age_column:
        ...
        estimated_age = estimate_age_simulation(
            attention,
            alpha,
            beta
        )
        ...
        st.markdown(
            ...
            'Actual trained model will be connected in Step B'
            '</div>',
            unsafe_allow_html=True
        )
